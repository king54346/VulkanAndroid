...
object ShaderLoader {
    ...
    fun loadVertexShader(context: Context): ByteArray {
        return loadShader(context, "shaders/demo_vert.spv")  // 使用全屏三角形顶点着色器
    }

    @Throws(IOException::class)
    fun loadFragmentShader(context: Context): ByteArray {
        return loadShader(context, "shaders/demo_frag.spv")  // 使用Push Constants测试着色器
    }
}
...
class SimpleVulkanFilter(private val context: Context) : VulkanFilter {
    ...
    companion object {
        private const val TAG = "SimpleVulkanFilter"
