...
Java_com_genymobile_scrcpy_vulkan_SimpleVulkanFilter_nativeCreatePipelineLayout(
    ...
    LOGI("✓ Pipeline layout created with descriptor set: %p", (void*)pipelineLayout);
    return reinterpret_cast<jlong>(pipelineLayout);
}

// 创建Pipeline ，pipelayout+rendpass
extern "C" JNIEXPORT jlong JNICALL
...
Java_com_genymobile_scrcpy_vulkan_SimpleVulkanFilter_nativePushConstants(
    ...
    if (dataSize != 4) {
        LOGE("Invalid data size: %d (expected 4)", dataSize);
        return;
    }
    ...
    vkCmdPushConstants(
            commandBuffer,
            pipelineLayout,
            VK_SHADER_STAGE_FRAGMENT_BIT,  // 关键：片元着色器
            0,
    ...
    if (logCounter++ % 100 == 0) {  // 每100帧打印一次
        LOGI("Pushed constants: res=(%.0f, %.0f), time=%.2f",
