...
object AffineShaderLoader {
    ...
    fun loadVertexShader(context: Context): ByteArray {
        // 使用支持变换矩阵的顶点着色器
        return loadShader(context, "shaders/affine_vert.spv")
    }

    @Throws(IOException::class)
    fun loadFragmentShader(context: Context): ByteArray {
        // 使用支持边界裁剪的片段着色器
        return loadShader(context, "shaders/affine_frag.spv")
    }
}
...
class AffineVulkanFilter(
    ...
    override fun init(device: Long, renderPass: Long) {
        ...
        try {
            // 1. Load shaders
            val vertexShaderCode = AffineShaderLoader.loadVertexShader(context)
            val fragmentShaderCode = AffineShaderLoader.loadFragmentShader(context)
